...
$(function(){
	...
	$('#btn').click(function() {
		$.ajax({
			...
			data: {'lane':$('#lane').val()},
			dataType : 'json',
			success : function(damagetaken) {
				showChart(damagetaken);
			},
			error : function(err) {
				console.log(err);
			}
...
//구글 챠트 라이브러리
function showChart(damagetaken){
	...
	function drawVisualization() {
	         ...
	         let arr=[];
	         arr.push(['챔피언이름', '피해량',{ role: 'style'}, {role:'annotation'}]);
	          for(let elem of damagetaken){
